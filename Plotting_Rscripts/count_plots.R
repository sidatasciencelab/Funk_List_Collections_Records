...
#### read in funk list ####
funk_list_original <- read.csv("assets/funk_list.csv", header = T)
# filter for deceased and save copy
funk_list_deceased <- funk_list_original %>% dplyr::filter(Deceased..Current..Recent == "D") 
write.csv(funk_list_deceased, file = "funk_list_deceased.csv", row.names = F)
...
file_list <- list.files('data/bionomia', full.names=TRUE)
...
write.csv(file = "bionomia_df.csv", bionomia_df, row.names = F)
...
# filter for deceased 
funk_list_D <- funk_list %>% dplyr::filter(Deceased..Current..Recent == "D")
# save a copy of this csv
write.csv(file = "funk_list_deceased.csv", funk_list_D, row.names = F)

# remove those with no counts
funk_list_D_counts <- funk_list_D %>% drop_na(count) %>% dplyr::filter(Deceased..Current..Recent == "D")

# count collections per SI unit and sort for plotting purposes
SI.UNIT_counts <- funk_list_D_counts %>% dplyr::group_by(SI.UNIT) %>% dplyr::summarize(sum_count = sum(count, na.rm = T), ) %>% arrange((sum_count))
unit_sorted <- SI.UNIT_counts$SI.UNIT

# make units a factor
funk_list_D_counts$SI.UNIT <- factor(funk_list_D_counts$SI.UNIT, levels = unit_sorted)



# # make a combined count column, treating NAs as 0s
# funk_list_D_counts %>% 
#   rowwise() %>% 
#   mutate(count = sum(count_recordedBy, count_identifiedBy, na.rm = TRUE)) -> funk_list_D_counts
...
##### plot counts #####  
unit_colors <- rev(brewer.pal(n = length(unit_sorted[unit_sorted %in% funk_list_D_counts$SI.UNIT]), name = 'Spectral'))
# reorder the colors
first <- unit_colors[1]
...
count_plot <-   ggdotchart(funk_list_D_counts, x = "full_name", y = "count",
                                        ...
                                        dot.size = 2.5,                                    
                                        label = round(funk_list_D_counts$count),           # Add counts values as dot labels
...
for (i in 1:length(unit_sorted)) {
  width[i] <- length((funk_list_D_counts %>% filter(SI.UNIT == unit_sorted[i]))$count)
...
unit_rects_xy <- data.frame(SI.UNIT = unit_sorted, xmin = xmin, xmax = xmax, width = width, color = color, x_text = x_text, y_text = rep((max(funk_list_D_counts$count))*1.2))

add_rects <- function(yx){
  annotate(geom = "rect", 
           ymin = -1000,
           ymax = (max(funk_list_D_counts$count))*1.25,
           xmin = yx$xmin, 
           xmax = yx$xmax,
           alpha = 0.15,
           fill = yx$color) 
...
pdf(file = 'funk_counts.pdf', height = 5.5, width = 3.25)
count_plot_rects
dev.off()
...
funk_list_D_counts_NMNH <- funk_list_D_counts %>% dplyr::filter(SI.UNIT == "NMNH") 

# count collections per Department in NMNH and sort for plotting purposes
DEPARTMENT_counts_NMNH <- funk_list_D_counts_NMNH %>% group_by(DEPARTMENT) %>% dplyr::summarize(sum_count = sum(count, na.rm = T), ) %>% arrange((sum_count)) 
dept_sorted_NMNH <- DEPARTMENT_counts_NMNH$DEPARTMENT

# make departments a factor 
funk_list_D_counts_NMNH$DEPARTMENT <- factor(funk_list_D_counts_NMNH$DEPARTMENT, levels = dept_sorted_NMNH) 

dept_colors_NMNH <- (brewer.pal(n = length(dept_sorted_NMNH[dept_sorted_NMNH %in% funk_list_D_counts_NMNH$DEPARTMENT]), name = 'Spectral'))
# dept_colors_NMNH <- c(dept_colors_NMNH, dept_colors_NMNH)

count_plot_NMNH <-   ggdotchart(funk_list_D_counts_NMNH, x = "full_name", y = "count",
                                             ...
                                             dot.size = 2.75,                                    
                                             label = round(funk_list_D_counts_NMNH$count),           # Add counts values as dot labels
...
for (i in 1:length(dept_sorted_NMNH)) {
  width[i] <- length((funk_list_D_counts_NMNH %>% filter(DEPARTMENT == dept_sorted_NMNH[i]))$count)
  xmax[i] <- cumsum(width)[i] + 0.5
  xmin[i] <- cumsum(width)[i] - width[i] + 0.5
...
# make a df of dimensions
dept_rects_xy_NMNH <- data.frame(DEPARTMENT = dept_sorted_NMNH, xmin = xmin, xmax = xmax, width = width, color = color, x_text = x_text, y_text = rep((max(funk_list_D_counts_NMNH$count))*1.2))


add_rects <- function(yx){
  annotate(geom = "rect", 
           ymin = -1000,
           ymax = (max(funk_list_D_counts_NMNH$count))*1.25,
           xmin = yx$xmin, 
           xmax = yx$xmax,
           alpha = 0.15,
           fill = yx$color) 
...
pdf(file = 'funk_counts_nmnh.pdf', height = 5.5, width = 3.25)
...
# filter for deceased and remove those with no counts, and sub labels
funk_list_D_counts_collectionCode <- funk_list_collectionCode %>% dplyr::filter(Deceased..Current..Recent == "D") 

# at this point NA in counts_collectionCode means there were no collections found for that person, so can omit them
funk_list_D_counts_collectionCode <- funk_list_D_counts_collectionCode %>% drop_na(count_collectionCode)

# replace blanks in collectionCode column with NA then NA with "Unknown"
funk_list_D_counts_collectionCode$collectionCode_edit <- as.character(funk_list_D_counts_collectionCode$collectionCode_edit)
funk_list_D_counts_collectionCode$collectionCode_edit[funk_list_D_counts_collectionCode$collectionCode_edit==""] <- NA
funk_list_D_counts_collectionCode$collectionCode_edit[is.na(funk_list_D_counts_collectionCode$collectionCode_edit)] <- "Unknown"

# count collections per Department in NMNH and sort for plotting purposes
collectionCode_counts <- funk_list_D_counts_collectionCode %>% dplyr::group_by(collectionCode_edit) %>% dplyr::summarize(sum_count = sum(count_collectionCode, na.rm = T), ) %>% arrange((sum_count)) 
collectionCode_sorted <- collectionCode_counts$collectionCode_edit
# set factor levels to this sort order
funk_list_D_counts_collectionCode$collectionCode_edit <- factor(funk_list_D_counts_collectionCode$collectionCode_edit, levels = collectionCode_sorted) # levels 

# sort names by total collectioons
names_collectionCode_counts <- funk_list_D_counts_collectionCode %>% dplyr::group_by(full_name) %>% dplyr::summarize(count_sum = sum(count_collectionCode)) %>% arrange(count_sum) %>% distinct()
names_collectionCode_sorted <- unique(names_collectionCode_counts$full_name)
# set factor levels to this sort order
funk_list_D_counts_collectionCode$full_name <- factor(funk_list_D_counts_collectionCode$full_name, levels = names_collectionCode_sorted)
...
funk_list_D_counts_collectionCode_4 <- funk_list_D_counts_collectionCode %>% filter(name %in% collectionCode_4$name)
# make df for xy coords
asterisk_4 <- funk_list_D_counts_collectionCode_4 %>% dplyr::group_by(full_name) %>% dplyr::summarize(sum_count = sum(count_collectionCode)) 

# add asterisks to this plot 
ggplot(funk_list_D_counts_collectionCode, aes(fill=collectionCode_edit, y=count_collectionCode, x=full_name)) + 
...
pdf(file = 'collectionCode_4_bars.pdf', height = 3, width = 3.25)
...
# filter for deceased and remove those with no counts, and sub labels
funk_list_D_counts_family <- funk_list_family  %>% dplyr::filter(Deceased..Current..Recent == "D") 

# at this point NA in counts_family means there were no collections found for that person, so can omit them
funk_list_D_counts_family <- funk_list_D_counts_family %>% drop_na(count_family)

# replace blanks in family column with NA then NA with "Unknown"
funk_list_D_counts_family$family <- as.character(funk_list_D_counts_family$family)
funk_list_D_counts_family$family[funk_list_D_counts_family$family==""] <- NA
funk_list_D_counts_family$family[is.na(funk_list_D_counts_family$family)] <- "Unknown"

# # # count collections per Department in NMNH and sort for plotting purposes
# # family_counts <- funk_list_D_counts_family %>% group_by(family) %>% dplyr::summarize(sum_count = sum(count_family, na.rm = T), ) %>% arrange(desc(sum_count)) 
# # family_sorted <- family_counts$family
# # set factor levels to this sort order
# funk_list_D_counts_family$family <- factor(funk_list_D_counts_family$family, levels = family_sorted) # levels 

# # sort names by total number of families
# names_family_counts <- funk_list_D_counts_family %>% group_by(name_readable) %>% summarize(count_sum = sum(count_family)) %>% arrange(count_sum)
# names_family_sorted <- names_family_counts$name_readable
# # set factor levels to this sort order
# funk_list_D_counts_family$name_readable <- factor(funk_list_D_counts_family$name_readable, levels = names_family_sorted)

# count number of different familys each person collected to identify broad collectors
family_botany <- funk_list_D_counts_family %>% ungroup() %>% group_by(full_name) %>% dplyr::summarize(families_count = length(unique(family))) %>% arrange((families_count)) %>% unique()
...
# plot
ggplot(family_botany, aes(y=families_count, x=full_name)) + 
  geom_bar(position="stack", stat="identity", alpha = 0.75, fill = "#03CEA4") + 
  coord_flip() +
  theme_classic() +
  theme(axis.text.x = element_text(size = 5),
...
pdf(file = 'family_counts.pdf', height = 4, width = 2.2)
family_bars
dev.off()
